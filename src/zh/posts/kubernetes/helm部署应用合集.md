...
- helm
- kubernetes
date: 2023-08-16 14:05:21
---
## helm部署pxc-db
pxc-operator: https://artifacthub.io/packages/helm/percona/pxc-operator
pxc-db: https://artifacthub.io/packages/helm/percona/pxc-db
~~~shell
helm repo add percona https://percona.github.io/percona-helm-charts
helm repo update
helm install mysql-pxc-op percona/pxc-operator --namespace db --create-namespace
touch pxc-mysql.values.yaml
helm install -f pxc-mysql.values.yaml mysql-pxc-db percona/pxc-db --namespace db --create-namespace
~~~
pxc-mysql.values.yaml
具体配置项目见官网描述(Custom Resource options reference): https://docs.percona.com/percona-operator-for-mysql/pxc/operator.html
~~~yaml
...
~~~
验证 fs3NDn~Mk6jzZPM)c(H
~~~shell
ROOT_PASSWORD=`kubectl -n db get secrets mysql-pxc-db-secrets -o jsonpath="{.data.root}" | base64 --decode`
kubectl -n db exec -ti mysql-pxc-db-pxc-0 -c pxc -- mysql -uroot -p"$ROOT_PASSWORD"
kubectl run -i --rm --tty percona-client --image=percona:8.0 --restart=Never -- bash -il
mysql -h mysql-pxc-db-proxysql.db.svc.cluster.local -uroot -pfs3NDn~Mk6jzZPM)c(H
~~~
pxc操作 参考:https://docs.percona.com/percona-operator-for-mysql/pxc/api.html
备份和恢复 参考:https://docs.percona.com/percona-operator-for-mysql/pxc/backups-restore-to-new-cluster.html
~~~shell
#列出 Percona XtraDB 集群
